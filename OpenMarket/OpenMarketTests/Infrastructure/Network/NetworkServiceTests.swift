...
//  OpenMarketTests
//
//  Created by Derrick kim on 2023/01/26.
//
...
final class NetworkServiceTests: XCTestCase {
    func test_Mock데이터를받을때_정확한Response가반환되어야한다() async {
        ...
        let expectation = self.expectation(description: "정확한 데이터가 반환되어야 한다.")
        ...
        //when
        do {
            let responseData = try await sut.request(endpoint: EndpointMock(path: baseURL,
                                                                            method: .get))
    ...
    func test_Error가_NSURLErrorCancelled로_반환될때_CancelledError를반환해야한다() async {
        //given
        let config = NetworkConfigurableMock()
        let expectation = self.expectation(description: "hasStatusCode error가 반환되어야 한다.")
        let cancelledError = NSError(domain: "network", code: NSURLErrorCancelled, userInfo: nil) as Error
        let sut = DefaultNetworkService(config: config,
        ...
        //when
        do {
            _ = try await sut.request(endpoint: EndpointMock(path: "https://openmarket.yagom-academy.kr",
                                                             method: .get))
            XCTFail("발생하면 안된다.")
        } catch let error {
