...
extension UIImage {
    func compress() -> Data? {
        guard var compressedImage = self.jpegData(
            compressionQuality: 0.2) else {
            return nil
        }

        while compressedImage.count > 307200 {
            compressedImage = UIImage(
                data: compressedImage)?.jpegData(compressionQuality: 0.5) ?? Data()
        }
    ...
    func configureSquareImageView() -> UIView {
        let view = UIView()
        let imageView = configureProfileImageView(
            with: self)
    ...
    private func configureProfileImageView(
        with image: UIImage) -> UIImageView {
